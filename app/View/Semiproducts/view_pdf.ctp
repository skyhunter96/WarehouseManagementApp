...
$pdf->Cell(0,0, "Kartica Artikla - Poluproizvod", 0,1,'C');
...
$html = '<br><br><br><br><table border="1" cellpadding="2" cellspacing="2" align="center"> <tr><th>Ime artikla:</th>
		<th>Kod artikla:</th><th>Status artikla:</th><th>Za servisnu produkciju:</th>
	</tr>';

foreach($data as $d){
	$serviceP = $d['Semiproduct']['is_for_service_production'] ? 'Da' : 'Ne';
	$html .= "
					<tr>
						<td>{$d['Item']['name']}</td>
						<td>{$d['Item']['code']}</td>
						<td>{$d['Semiproduct']['status']}</td>
						<td>{$serviceP}</td>
						<td></td>
					</tr><br>
				 ";
